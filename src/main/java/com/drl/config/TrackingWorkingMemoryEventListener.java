...
public class TrackingWorkingMemoryEventListener extends DefaultRuleRuntimeEventListener {
	...
	public void objectInserted(final ObjectInsertedEvent event) {
		if ((handleFilter == null && classFilter == null) || event.getFactHandle() == handleFilter
				|| event.getObject().getClass().equals(classFilter)) {
			insertions.add(event);
			allEvents.add(event);
			log.info("Insertion: " + CommonUtils.objectDetails(event.getObject()));
		}
	}
	...
	public void objectDeleted(final ObjectDeletedEvent event) {
		if ((handleFilter == null && classFilter == null) || event.getFactHandle() == handleFilter
			...
			deletions.add(event);
			allEvents.add(event);
			log.info("Retraction: " + CommonUtils.objectDetails(event.getOldObject()));
		}
	}
	...
	public void objectUpdated(final ObjectUpdatedEvent event) {
		if ((handleFilter == null && classFilter == null) || event.getFactHandle() == handleFilter
			...
			try {
				...
				log.error("Unable to get object details for tracking: " + CommonUtils.objectDetails(fact), e);
			}
			log.info("Update: " + CommonUtils.objectDetails(event.getObject()));
		}
	}
